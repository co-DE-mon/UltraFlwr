...
def create_yolo_structure(data_home, output_dir):
    ...
    unique_frames = {"train": set(), "val": set(), "test": set()}  # frames that have at least 1 bbox
    ...
    for split in ["train", "val", "test"]:
        ...
        for annotation in coco_data["annotations"]:
            ...
            with open(label_file_path, 'a') as f_label:
                f_label.write(f"{class_id} {x_center:.6f} {y_center:.6f} {w_norm:.6f} {h_norm:.6f}\n")
    ...
    generate_data_yaml(
        output_dir, 
        ["train", "val", "test"], 
        coco_data["categories"],  # from the last loaded coco_data
        total_frames, 
        unique_frames, 
        duplicates_count, 
        total_bboxes, 
    ...
    for split in ["train", "val", "test"]:
        ...
        print(f"  - Images with no annotations:     {len(frames_no_annotations[split])}")
...
def generate_data_yaml(
    ...
    for split in splits:
        ...
        yaml_content[f"{split}_frames_no_annotations"] = len(frames_no_annotations[split])
