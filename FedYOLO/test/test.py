...
def get_server_metrics(weights_path, dataset_name):
    ...
    server_model_metrics = server_model.val(data=dataset_name, verbose=False)
...
server_model_global_dataset = create_safe_dataframe(server_model_global_table, 'global')

# Merge all tables
server_model_combined_table = server_model_client0_table.merge(
    server_model_client1_table, on='Class', how='outer'
).merge(
    server_model_global_table, on='Class', how='outer'
)
