...
# ------------Modified for Proximal Detection Loss----------------
...
class ProximalDetectionLoss(v8DetectionLoss):
    ...
    def __init__(self, model, global_params, proximal_mu):
        ...
        self.model = model  # de-parallelized model
        ...
        if self.global_params:
            ...
            for name, param in self.model.named_parameters():
                if name in self.global_params:
                    proximal_term += torch.sum((param - self.global_params[name].to(param.device)) ** 2)
            
            # Scale by proximal_mu/2
            proximal_term = self.proximal_mu * proximal_term / 2.0
            
            # Add to task loss
            task_loss += proximal_term
    ...
    def update_global_params(self, global_params):
        ...
        self.global_params = global_params
        self.proximal_mu = global_params.get('proximal_mu', self.proximal_mu)
